...
if (risk?.level === "RED") {
  ...
  autoTable(doc, {
    ...
    ["Audio Peak", vital.audio ?? "—", "—", "Not Applicable"],
    theme: "grid",
...
// ── Force heading + table together on same page ──
const heartTableH = 9 * 9 + 20; // approximate height
if (y + heartTableH > doc.internal.pageSize.getHeight() - 30) {
  doc.addPage();
  y = 20;
}
    doc.setFont("helvetica", "bold");
    ...
    const lungHeaderColor: [number, number, number] = lungAbnormal
      ...
      : [21, 128, 61];  // green if normal
// ── Force Lung heading + table on same page ──
const approxLungH = 5 * 11 + 20;
if (y + approxLungH > doc.internal.pageSize.getHeight() - 30) {
  doc.addPage();
  y = 20;
}
...
{(profileRole === "admin" || profileRole === "user") && (
